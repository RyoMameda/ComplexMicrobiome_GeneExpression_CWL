...
cut -f1 ${f}_rRNAlist.txt | sort > ${f}_rRNA_toplist.txt
...
awk '!x[$1]++' ${f}-rRNA_uniprot.txt | cut -f1 | sort > ${f}-rRNA_uniprot_toplist.txt
# cat toplist.txt ${f}_list-rRNA.txt | sort | uniq -u > ${f}_list-rRNA-uniprot.txt
seqkit grep -v -f ${f}_list-rRNA-uniprot.txt ${f}-rRNA.faa > ${f}-rRNA-uniprot.faa
#rm toplist.txt ${f}_list-rRNA-uniprot.txt
rm ${f}-rRNA_uniprot_toplist.txt
